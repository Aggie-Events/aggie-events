...
export default function LoginScreen({
  ...
  const handleLogout = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_AUTH_URL}/logout`,
        {
          method: "POST",
          credentials: "include",
        },
  ...
  const handleUsernameSelectionClose = async () => {
    ...
    ToastManager.addToast(
      "Username selection cancelled. Please try signing in again.",
      "error",
      3000,
    );
    if (onClose) onClose();
  ...
  const handleGoogleAuth = async () => {
    setIsLoading(true);
    try {
      const user = await openAuthPopup(
      ...
      >
        ...
        <div className="space-y-4">
          ...
          >
            <FcGoogle
              size={20}
              className={`transition ${isLoading ? "" : "group-hover:scale-110"}`}
            />
            <span>
              {isLoading
                ? "Connecting..."
